...
public class HumanController2 : MonoBehaviour
{
    ...
    void Update()
    {
        gameObject.GetComponentInChildren <Animator>().SetBool("waiting", false);
        if (waitTimer > 0)
        {
            waitTimer -= 1;
            Debug.Log("I'm waiting");
            gameObject.GetComponentInChildren<Animator>().SetBool("waiting", true);
            agent.SetDestination(gameObject.transform.position);
            return;
        }
        
        if (agent.velocity.magnitude <= 1 && !pursuing)
        {
            Debug.Log("I'm going somewhere else");
